...
http.interceptor.request((config, cancel) => {
	...
	// 	config.baseUrl = 'http://' + uni.getStorageSync('iphost') + '/'
	// }
	if (uni.getStorageSync('base_url')) {
		// config.baseUrl = uni.getStorageSync('base_url') + '/wms/'
		config.baseUrl = '/'
	}
	let userinfor = uni.getStorageSync('login_user_info')
	config.header = {
		...config.header,
	...
	let tenantId = userinfor.relTenantIds
	if (!tenantId) {
		tenantId = 0;
	}
	...
	if (response) {
		...
		switch (data.status) {
			case 403:
				...
				break
			case 500:
				if (!token || data.message.includes('Token失效，请重新登录')) {
