<template>
	<view class="content">
		<cu-custom bgColor="head2" :isBack="true">
			...
			<block slot="content">{{$t('装车复核')}}</block>
		</cu-custom>
		...
		<scroll-view scroll-y="true" style="flex: 1;overflow: auto;" @scrolltolower='scroll'>
			<view class="body_t" v-for="(item,index) in data" :key='index'>
				<view class="item">
					...
					<view style="color: #2D2F2E;max-width: 70%;">{{item.orderId}}</view>
				</view>
				...
				<view class="item">
					...
					<view style="color: #2D2F2E;max-width: 70%;">{{item.kuWeiBianMa}}</view>
				</view>
				

				<view class="item">
					<view>{{$t('pages.数量')}}：</view>
					<view style="color: #2D2F2E;max-width: 70%;">{{item.goodsQua}}</view>
				</view>
				
				<view class="item">
					<view>{{$t('pages.单位')}}：</view>
					<view style="color: #2D2F2E;max-width: 70%;">{{item.shlDanWei}}</view>
				</view>

				<view class="inputText">
					<view>{{$t('pages.复核数量')}}：</view>
					<input v-model="conditionObj[`query05${index}`]" class="inputText_itm" :focus="Focus5" />
				</view>


				<view class="btn" @click="preserve(item,index)">{{$t('确认')}}</view>
			</view>
		</scroll-view>
	</view>
</template>
	...
	export default {
		...
		methods: {
			getDetails() { // 获取装车复核列表
				...
				this.$http.get('/jeewms/rest/pdaapi/wvGiController/list', {
					params
				}).then(res => {
			...
			preserve(item, index) {
				let data = {
					...
					id: item.id,
					goodsQuaok: this.conditionObj[`query05${index}`]
				}
				if(!data.goodsQuaok) {
					uni.showToast({title: '请输入复核数量' ,icon:'none'})
				}
				let url = `/jeewms/rest/pdaapi/wmToDownGoodsController/change?wmToDownGoodsstr=${JSON.stringify(data)}`
				let that = this
				that.goOn(data, url)
			},
