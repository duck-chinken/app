	...
	<view class="content">
		...
		<scroll-view scroll-y="true" style="flex: 1;overflow: auto;" @scrolltolower='scroll'>
			<view class="body_t" v-for="(item,index) in data" :key=	'index' >
				...
				<view class="item">
					<view>库存数量：</view>
					<view style="color: #2D2F2E;">{{item.goodsQua}}/{{item.goodsUnit}}</view>
				</view>
				<view class="item">
					<view>移库数量：</view>
					<input type="text" value="" v-model="item.goodsqua"/>
				</view>
				<view class="item">
					<view>到储位：</view>
					<input type="text" value="" v-model="item.binto"/>
				</view>
				
				<view class="item">
					<view>到托盘：</view>
					<input type="text" value="" v-model="item.tinto"/>
				</view>
				<view class="btn" @click="preserve(item,index)">{{$t('确定')}}</view>
			</view>
		</scroll-view>
	</view>
</template>
...
<script>
	export default {
		...
		methods: {
			preserve(item, index) {
				if(!item.goodsqua) {
					return uni.showToast({title:'请输入移库数量',icon:'none'})
				}
				if(!/^\d*(\.)?\d*$/.test(item.goodsqua)){
					return uni.showToast({title:'移库数量请输入纯数字',icon:'none'})
				}
				let data = {
				...
				let url = `/jeewms/rest/pdaapi//wvStockController/pdazysave?username=${this.userName}&id=${item.id}&
							goodsqua=${item.goodsqua}&`
				if(item.binto){
					url += `binto=${item.binto}&`
				}
				if(item.tinto){
					url += `tinto=${item.tinto}`
				}
				let that = this
				that.goOn(data, url)
			},
			...
			getDetails(e) { // 获取收货列表
				...
				this.$http.get('/jeewms/rest/pdaapi/wvStockController/list', {
					...
					this.data = [...this.data,...res.data.obj] 
					if(!this.data.length) {
						this.query01 = ''
						this.query02 = ''
					}
