<template>
	<view class="content">
		...
		<view class="Search" style="flex-direction: column;height: 	177rpx;">
			...
			<input type="text" v-model="query02" :focus="isFocus1"  @confirm="getDetails(2)" placeholder="请扫描一次容器" style="margin-top: 16rpx;" />
		</view>
		<scroll-view scroll-y="true" style="flex: 1;overflow: auto;" @scrolltolower='scroll'>
			<view class="body_t" v-for="(item,index) in data" :key=	'index'>
				...
				<view class="item">
					<view>一次容器：</view>
					<view style="color: #3297E2;">{{item.firstRq}}</view>
				</view>
				<view class="inputText">
					<view class="" style="display: flex; width: 100%; align-items: center;justify-content: space-between;">
						<text>分拣容器：</text>
						<input v-model="conditionObj['secondRq']" style=" color: #3297E2;text-align: right;padding-right: 20rpx;width: 300rpx;margin-top: 0;" type="number" value=""  placeholder-style="color:#E0E2E4"/>
					</view>
				</view>
				<view class="btn"  @click="preserve(item)">分拣</view>
			</view>
		</scroll-view>
	</view>
</template>
		...
		methods: {
			...
			preserve(item, index) {
				let data = {
			        ...
			        secondRq: this.conditionObj['secondRq']
				}
				if(!data.secondRq) {
					uni.showToast({title: '请输入二次容器' ,icon:'none'})
				}
				let url = `/jeewms/rest/pdaapi/waveToFjController/save?waveToFjstr=${JSON.stringify(data)}`
				let that = this
				that.goOn(data, url)
			},
			...
			getDetails(e){
				...
				this.$http.get('/jeewms/rest/pdaapi/waveToFjController/list/tofj',{params:data}).then(res => {
